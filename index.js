...
myApp.get("/", function (req, res) {
  res.render("form"); // will render views/home.ejs
});
// render the login page
myApp.get("/login", function (req, res) {
  res.render("login"); // will render views/login.ejs
});
myApp.get("/Submit", function (req, res) {
  res.render("Submit"); // will render views/login.ejs
});
...
myApp.get("/print/:cardid", function (req, res) {
  ...
  Card.findOne({ _id: cardId }).exec(function (err, card) {
    res.render("View", card); // render card.ejs with the data from card
  });
});
...
myApp.post("/editprocess/:cardid", function (req, res) {
  if (!req.session.loggedIn) {
    ...
    Card.findOne({ _id: cardId }).exec(function (err, card) {
      ...
      card.save();
      res.render("editSuccess", card); // render card.ejs with the data from card
    });
  }
});
...
myApp.post(
  ...
  [
    ...
    check("scaryName", "Please enter firstname and lastname").matches(
      nameRegex
    ),
  ],
  function (req, res) {
